...
export function CatalogView({ selectedCategory, onProductSelect, onViewChange }: CatalogViewProps) {
  ...
  const { categories, products, loading } = useCatalogData();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200">
        {loading && <div className="h-1 bg-[#0066b3]/20"><div className="h-full w-1/3 bg-[#0066b3] animate-pulse" /></div>}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            {totalItems > 0 && (
              <Button onClick={onViewChange} className="bg-green-600 hover:bg-green-700 text-white">
              ...
              <div className="space-y-2">
                <button
                  ...
                  className={`w-full text-left px-4 py-2 rounded-lg text-sm transition-colors ${
                    activeCategory === null ? 'bg-[#0066b3] text-white font-medium' : 'text-gray-600 hover:bg-gray-100'
                  }`}
                >
                ...
                {categories.map((category) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 rounded-lg text-sm transition-colors ${
                      activeCategory === category.id ? 'bg-[#0066b3] text-white font-medium' : 'text-gray-600 hover:bg-gray-100'
                    }`}
                  >
                    {category.name}
                  </button>
